...
namespace faster_gs::rasterization::kernels {
    ...
    // used when sh coefficients are supplied jointly
        ...
        const uint total_sh_bases)
    {
        ...
        const float3* coefficients_ptr = sh_coefficients + primitive_idx * total_sh_bases;
        float3 result = 0.5f + C0 * coefficients_ptr[0];
        if (active_sh_bases > 1) {
            auto [x, y, z] = normalize(position - cam_position);
            result = result - C1 * y * coefficients_ptr[1]
                            + C1 * z * coefficients_ptr[2]
                            - C1 * x * coefficients_ptr[3];
            if (active_sh_bases > 4) {
                const float xx = x * x, yy = y * y, zz = z * z;
                const float xy = x * y, xz = x * z, yz = y * z;
                result = result + C2a * xy * coefficients_ptr[4]
                                - C2a * yz * coefficients_ptr[5]
                                + (C2b * zz - C2c) * coefficients_ptr[6]
                                - C2a * xz * coefficients_ptr[7]
                                + C2d * (xx - yy) * coefficients_ptr[8];
                if (active_sh_bases > 9) {
                    result = result + y * (C3a * yy - C3b * xx) * coefficients_ptr[9]
                                    + C3c * xy * z * coefficients_ptr[10]
                                    + y * (C3d - C3e * zz) * coefficients_ptr[11]
                                    + z * (C3f * zz - C3g) * coefficients_ptr[12]
                                    + x * (C3d - C3e * zz) * coefficients_ptr[13]
                                    + C3h * z * (xx - yy) * coefficients_ptr[14]
                                    + x * (C3b * yy - C3a * xx) * coefficients_ptr[15];
                }
            }
        }
        return result;
    ...
    // used when sh coefficients are supplied separately
    __device__ inline float3 convert_sh_to_color(
        ...
        const uint total_sh_bases)
    {
        ...
        const uint coefficients_base_idx = primitive_idx * total_sh_bases;
        const float3* coefficients_ptr = sh_coefficients + coefficients_base_idx;
        float3* grad_coefficients_ptr = grad_sh_coefficients + coefficients_base_idx;
        ...
        grad_coefficients_ptr[0] = C0 * grad_color;
        ...
        if (active_sh_bases > 1) {
            ...
            grad_coefficients_ptr[1] = -C1 * y * grad_color;
            grad_coefficients_ptr[2] = C1 * z * grad_color;
            grad_coefficients_ptr[3] = -C1 * x * grad_color;
            float3 grad_direction_x = -C1 * coefficients_ptr[3];
            float3 grad_direction_y = -C1 * coefficients_ptr[1];
            float3 grad_direction_z = C1 * coefficients_ptr[2];
            if (active_sh_bases > 4) {
                const float xx = x * x, yy = y * y, zz = z * z;
                const float xy = x * y, xz = x * z, yz = y * z;
                grad_coefficients_ptr[4] = C2a * xy * grad_color;
                grad_coefficients_ptr[5] = -C2a * yz * grad_color;
                grad_coefficients_ptr[6] = (C2b * zz - C2c) * grad_color;
                grad_coefficients_ptr[7] = -C2a * xz * grad_color;
                grad_coefficients_ptr[8] = C2d * (xx - yy) * grad_color;
                grad_direction_x = grad_direction_x + C2a * y * coefficients_ptr[4]
                                                    - C2a * z * coefficients_ptr[7]
                                                    + C2a * x * coefficients_ptr[8];
                grad_direction_y = grad_direction_y + C2a * x * coefficients_ptr[4]
                                                    - C2a * z * coefficients_ptr[5]
                                                    - C2a * y * coefficients_ptr[8];
                grad_direction_z = grad_direction_z - C2a * y * coefficients_ptr[5]
                                                    + C2e * z * coefficients_ptr[6]
                                                    - C2a * x * coefficients_ptr[7];
                if (active_sh_bases > 9) {
                    grad_coefficients_ptr[9] = y * (C3a * yy - C3b * xx) * grad_color;
                    grad_coefficients_ptr[10] = C3c * xy * z * grad_color;
                    grad_coefficients_ptr[11] = y * (C3d - C3e * zz) * grad_color;
                    grad_coefficients_ptr[12] = z * (C3f * zz - C3g) * grad_color;
                    grad_coefficients_ptr[13] = x * (C3d - C3e * zz) * grad_color;
                    grad_coefficients_ptr[14] = C3h * z * (xx - yy) * grad_color;
                    grad_coefficients_ptr[15] = x * (C3b * yy - C3a * xx) * grad_color;
                    grad_direction_x = grad_direction_x - C3i * xy * coefficients_ptr[9]
                                                        + C3c * yz * coefficients_ptr[10]
                                                        + (C3d - C3e * zz) * coefficients_ptr[13]
                                                        + C3c * xz * coefficients_ptr[14]
                                                        + C3b * (yy - xx) * coefficients_ptr[15];
                    grad_direction_y = grad_direction_y + C3b * (yy - xx) * coefficients_ptr[9]
                                                        + C3c * xz * coefficients_ptr[10]
                                                        + (C3d - C3e * zz) * coefficients_ptr[11]
                                                        - C3c * yz * coefficients_ptr[14]
                                                        + C3i * xy * coefficients_ptr[15];
                    grad_direction_z = grad_direction_z + C3c * xy * coefficients_ptr[10]
                                                        - C3j * yz * coefficients_ptr[11]
                                                        + (C3k * zz - C3g) * coefficients_ptr[12]
                                                        - C3j * xz * coefficients_ptr[13]
                                                        + C3h * (xx - yy) * coefficients_ptr[14];
                }
            }
